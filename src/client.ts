...
function updatePlayerList(players: { nickname: string, score: number }[]) {
...
function handleGameStart(message: { drawer: string, word_length: number, round_duration: number }) {
    clearClientTimer();
...
function drawRemotely(data: { from: {x: number, y: number}, to: {x: number, y: number} }) {
    ctx.beginPath();
