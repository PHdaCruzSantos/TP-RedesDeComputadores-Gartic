...
1.  **Servidor TCP Explícito (Porta 2004):** O arquivo `server.ts` instancia um servidor TCP puro utilizando a biblioteca `net` do Node.js. Este servidor escuta na porta `2004` e está preparado para receber conexões TCP diretas. Isso demonstra o conhecimento e a aplicação direta da tecnologia de Sockets TCP.
